...
import { ArrowLeft, AlertCircle, RefreshCw, Share2 } from 'lucide-react';
import { Button } from '@/components/ui/button';
import { Skeleton } from '@/components/ui/skeleton';
import { useGetSharedAppCreation } from '../hooks/useQueries';
...
function getCreationTitle(content: string): string {
  try {
    const parsed = JSON.parse(content);
    return parsed?.data?.title || parsed?.title || parsed?.name || parsed?.prompt?.slice(0, 40) || 'Shared Creation';
  } catch {
    return 'Shared Creation';
  }
}
...
export default function SharedViewerPage() {
  ...
  const { data: item, isLoading, isError, refetch } = useGetSharedAppCreation(id);
    ...
    return (
      <div className="max-w-4xl mx-auto px-4 py-16 flex flex-col items-center text-center">
        ...
        <p className="text-sm text-muted-foreground mb-6 max-w-sm">
          This shared creation could not be found. It may have been removed or made private.
        </p>
  ...
  const title = getCreationTitle(item.content);
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 py-6">
      ...
      <p className="mt-4 text-center text-xs text-muted-foreground">
        This is a shared creation. Sign in to create your own.
      </p>
    </div>
  );
}
